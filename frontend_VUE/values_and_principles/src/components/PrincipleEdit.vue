<template>
  <div v-if="principle" style="overflow: hidden">
    <p>
      ...
      <br/>
      <input id="description" placeholder="Description" v-model="localPrinciple.description">
      <button @click="savePrinciple()" type="button" class="btn btn-primary">Save the modifications</button>
    ...
    savePrinciple() {
      if(this.principle.id){
        ...
        fetch(`http://127.0.0.1:8000/api/principles/${this.principle.id}/`, {
          ...
          body: JSON.stringify({description: this.localPrinciple.description})
        })
            .then( response => response.json())
            .then( response => {
              this.$emit('principle-updated')
