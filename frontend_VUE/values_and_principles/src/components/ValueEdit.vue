<template>
 <div v-if="value">
   <p>
     ...
     <label for="title" >{{value.title.charAt(0).toUpperCase() + value.title.slice(1)}}</label>
     <br/>
     <input id="title" placeholder="Title" v-model="localValue.title">
     <button @click="saveValue()" type="button" class="btn btn-primary">Save the modifications</button>
