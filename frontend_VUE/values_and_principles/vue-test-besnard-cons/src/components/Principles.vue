<template>
  <div class="list-group">
    ...
    <small class="text-muted"
      >Number of principles : {{ Object.keys(principles).length }} / 12</small
    >
    <PrincipleItem
      ...
      :key="principle.id"
      :principle="principle"
      @principleEdit="principleEdit($event)"
      @principleDelete="principleDelete($event)"
    />
    <br />
    <PrincipleEdit
      v-if="editedPrinciple"
      :principle="editedPrinciple"
      @principleUpdated="getPrinciples()"
    />
    <br />
    <h5 v-if="Object.keys(principles).length < 12">
      <font-awesome-icon class="plus" icon="plus" @click="newPrinciple()" />
...
export default {
  ...
  methods: {
    principleEdit(principle_id) {
      this.editedPrinciple = this.principles.find(
        (principle) => principle.id === principle_id
      );
